(function ($) {
  ...
  if (typeof shaOmnibusLowestPrice === "undefined") {
    return;
  }
  ...
  function cardLooksOnSale($card) {
    return (
      $card.find(
        ".price del, del .woocommerce-Price-amount, .onsale, .badge.sale, .badge-container .sale",
      ).length > 0
    );
  }
  ...
  function collectMissingCards(context) {
    ...
    $(".product, li.product, .product-small, .box-product", context).each(
      function () {
        var $card = $(this);
        ...
        if (
          !id ||
          $card.find(".sha-omnibus-lowest-price").length ||
          !cardLooksOnSale($card)
        ) {
          return;
        }
  ...
  function requestMissingLabels(context) {
    ...
    $.ajax({
      url: shaOmnibusLowestPrice.ajaxUrl,
      method: "POST",
      dataType: "json",
      data: {
        action: "sha_omnibus_get_labels",
        nonce: shaOmnibusLowestPrice.nonce,
  ...
  $(document).on(
    "click",
    ".ux-relay button, .ux-relay a, .ux-relay__button, .load-more, .load-more-button, .woocommerce-pagination a",
    function () {
