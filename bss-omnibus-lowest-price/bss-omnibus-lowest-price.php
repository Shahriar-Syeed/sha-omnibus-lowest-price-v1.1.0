<?php
/**
 * Plugin Name: BSS Omnibus Lowest Price for WooCommerce
 ...
 * Version: 1.1.0
 * Author: Bangladesh Software Solution (BSS)
 * Requires at least: 5.8
 * Requires PHP: 7.4
 * WC requires at least: 5.0
 * Text Domain: bss-omnibus-lowest-price
 */
...
final class BSS_Omnibus_Lowest_Price_For_WooCommerce {
    const VERSION       = '1.1.0';
    const OPTION_KEY    = 'bss_omnibus_lowest_price_options';
    const CRON_HOOK     = 'bss_omnibus_lowest_price_daily_record';
    const SEED_HOOK     = 'bss_omnibus_lowest_price_seed_prices';
    const NONCE_ACTION  = 'bss_omnibus_lowest_price_frontend';

    /**
     * @var BSS_Omnibus_Lowest_Price_For_WooCommerce|null
     */
    ...
    public static function defaults() {
        return array(
            'enabled'           => 'yes',
            'days'              => 30,
            'label'             => 'Lowest price in last 30 days:',
            'sale_only'         => 'yes',
    ...
    public static function table_name() {
        global $wpdb;
        return $wpdb->prefix . 'bss_omnibus_price_history';
    }
    ...
    public function init() {
        ...
        add_action( 'wp_ajax_bss_omnibus_get_labels', array( $this, 'ajax_get_labels' ) );
        add_action( 'wp_ajax_nopriv_bss_omnibus_get_labels', array( $this, 'ajax_get_labels' ) );
        ...
        add_shortcode( 'bss_omnibus_lowest_price', array( $this, 'shortcode_lowest_price' ) );
    }

    public function woocommerce_missing_notice() {
        if ( current_user_can( 'activate_plugins' ) ) {
            echo '<div class="notice notice-error"><p><strong>BSS Omnibus Lowest Price</strong> requires WooCommerce to be installed and active.</p></div>';
        }
    }
    ...
    public function register_settings() {
        register_setting(
            'bss_omnibus_lowest_price_settings',
            self::OPTION_KEY,
            array(
                'type'              => 'array',
    ...
    public function render_admin_page() {
        ...
        if ( isset( $_POST['bss_omnibus_seed_prices'] ) ) {
            check_admin_referer( 'bss_omnibus_seed_prices_action' );
        ...
        ?>
        <div class="wrap">
            <h1>BSS Omnibus Lowest Price</h1>

            <?php if ( null !== $seeded_count ) : ?>
                <div class="notice notice-success is-dismissible">
                    <p><?php echo esc_html( sprintf( 'Price history seeded/updated for %d product records.', $seeded_count ) ); ?></p>
                </div>
            <?php endif; ?>

            <p>
                This plugin records WooCommerce product prices and displays the lowest recorded price from the selected period for sale products.
                It also includes a Flatsome/UX Builder AJAX fallback for load-more product blocks.
            </p>

            <form method="post" action="options.php">
                <?php settings_fields( 'bss_omnibus_lowest_price_settings' ); ?>

                <table class="form-table" role="presentation">
                    <tr>
                        <th scope="row">Enable display</th>
                        <td>
                            <label>
                                <input type="checkbox" name="<?php echo esc_attr( self::OPTION_KEY ); ?>[enabled]" value="yes" <?php checked( $options['enabled'], 'yes' ); ?> />
                                Show Omnibus lowest price on the frontend
                            </label>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row">Tracking period</th>
                        <td>
                            <input type="number" min="1" max="365" name="<?php echo esc_attr( self::OPTION_KEY ); ?>[days]" value="<?php echo esc_attr( $options['days'] ); ?>" /> days
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description">Example: Lowest price in last 30 days:</p>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row">Sale products only</th>
                        <td>
                            <label>
                                <input type="checkbox" name="<?php echo esc_attr( self::OPTION_KEY ); ?>[sale_only]" value="yes" <?php checked( $options['sale_only'], 'yes' ); ?> />
                                Show only when the product is currently on sale
                            </label>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row">Flatsome AJAX fallback</th>
                        <td>
                            <label>
                                ...
                                Re-check newly loaded products after pagination/load more AJAX
                            </label>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row">Cart and checkout display</th>
                        <td>
                            <label>
                                ...
                                Show the lowest-price line in cart, mini-cart, and checkout rows
                            </label>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row">Product edit price history</th>
                        <td>
                            <label>
                                ...
                                Show price changes history inside the product price area
                            </label>
                            <p class="description">
                                Show last
                                ...
                                change records per product or variation.
                            </p>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row">Keep history for</th>
                        <td>
                            <input type="number" min="31" max="730" name="<?php echo esc_attr( self::OPTION_KEY ); ?>[cleanup_after_days]" value="<?php echo esc_attr( $options['cleanup_after_days'] ); ?>" /> days
                            <p class="description">Old history beyond this number of days is cleaned during daily recording.</p>
                        </td>
                    </tr>
                </table>
            ...
            <h2>Seed current prices</h2>
            <p>
                Use this once after activating the plugin. It records current product and variation prices as the starting point.
                The plugin cannot know historical prices from before installation unless they were already recorded by this plugin.
            </p>
            <form method="post">
                <?php wp_nonce_field( 'bss_omnibus_seed_prices_action' ); ?>
                <p><button type="submit" class="button button-secondary" name="bss_omnibus_seed_prices" value="1">Seed Current Prices Now</button></p>
            </form>
        </div>
        <?php
    }
    ...
    public function append_lowest_price_to_price_html( $price_html, $product ) {
        ...
        if ( false !== strpos( (string) $price_html, 'bss-omnibus-lowest-price' ) ) {
            return $price_html;
        }
    ...
    public function append_lowest_price_to_checkout_quantity( $quantity_html, $cart_item, $cart_item_key ) {
        ...
        if ( false !== strpos( (string) $quantity_html, 'bss-omnibus-lowest-price' ) ) {
            return $quantity_html;
        }
    ...
    private function get_lowest_price_label_html( WC_Product $product, $context = 'frontend' ) {
        ...
        return sprintf(
            '<small class="bss-omnibus-lowest-price bss-omnibus-lowest-price--%5$s" data-product-id="%1$d" data-record-product-id="%2$d"><span class="bss-omnibus-lowest-price__label">%3$s</span> <span class="bss-omnibus-lowest-price__amount">%4$s</span></small>',
    ...
    private function render_admin_price_history_panel( WC_Product $product, $context = 'simple' ) {
        ...
        $context_class = 'variation' === $context ? 'bss-omnibus-admin-history--variation' : 'bss-omnibus-admin-history--simple';
        ?>
        <div class="bss-omnibus-admin-history <?php echo esc_attr( $context_class ); ?>">
            <details>
                <summary>
                    <strong>Omnibus price changes history</strong>
                    <?php if ( $lowest && isset( $lowest->price ) ) : ?>
                        <span class="bss-omnibus-admin-history__lowest">
                ...
                <?php if ( empty( $rows ) ) : ?>
                    <p class="bss-omnibus-admin-history__empty">
                        No price history recorded yet. Update this product or use WooCommerce → BSS Omnibus Price → Seed Current Prices Now.
                    </p>
                <?php else : ?>
                    <table class="widefat striped bss-omnibus-admin-history__table">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Price</th>
    ...
    private function render_admin_history_styles_once() {
        ...
        ?>
        <style>
            .bss-omnibus-admin-history{clear:both;margin:12px 12px 14px;padding:10px 12px;border:1px solid #dcdcde;border-radius:4px;background:#fff;box-sizing:border-box}.bss-omnibus-admin-history--simple{margin-left:0;margin-right:0}.bss-omnibus-admin-history summary{cursor:pointer;display:flex;gap:10px;align-items:center;flex-wrap:wrap}.bss-omnibus-admin-history__lowest{font-size:12px;color:#1d2327;font-weight:500}.bss-omnibus-admin-history__empty{margin:10px 0 0;color:#646970}.bss-omnibus-admin-history__table{margin-top:10px}.bss-omnibus-admin-history__table th,.bss-omnibus-admin-history__table td{font-size:12px;padding:6px 8px}.woocommerce_variation .bss-omnibus-admin-history{margin-top:8px;width:calc(100% - 24px)}
        </style>
        <?php
    }
...
register_activation_hook( __FILE__, array( 'BSS_Omnibus_Lowest_Price_For_WooCommerce', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'BSS_Omnibus_Lowest_Price_For_WooCommerce', 'deactivate' ) );

BSS_Omnibus_Lowest_Price_For_WooCommerce::instance();
